...
class TD3(object):
    def __init__(
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, s_dim * 2 + a_dim + 7), dtype=np.float32)
    ...
    def store_transition(self, s, a, r, s_, done,flag_success_reward, flag_invalid_goal,flag_invalid_joints, flag_path_tolerance_violation, flag_goal_tolerance_violation):

        ##horizontally stacking the transitions in replay buffer
        transition = np.hstack((s, a, [r], s_, [done],[flag_success_reward], [flag_invalid_goal],[flag_invalid_joints], [flag_path_tolerance_violation], [flag_goal_tolerance_violation]))
