...
train_reward_filename = os.path.join(base_log_dir, 'Temp_training_rewards_whole.txt')
test_reward_filename = os.path.join(base_log_dir, 'Temp_testing_rewards_whole.txt')
joint_angles_filename = os.path.join(base_log_dir, 'Temp_Joint_Angles_whole.txt')
rpy_filename = os.path.join(base_log_dir, 'Temp_RPY_whole.txt')
...
def train(args, env, model):
    ...
    for epoch in range(0, 1000):
        ...
        if args.model_name == 'TD3':
            ...
            random_noises = np.random.uniform(-MAX_NOISE, MAX_NOISE, (max_ts, 11))
            previous_latency = 0
        ...
        for i in range(args.train_step):
            ...
            if args.model_name == 'TD3':
                ...
                state_, reward, terminal, target_pos, end_eff_pos, present_joint_angles, rpy, flag_success_reward, flag_invalid_goal,flag_invalid_joints, flag_path_tolerance_violation, flag_goal_tolerance_violation = env.step(action, object_position)
                model.store_transition(state,action,reward,state_,terminal, flag_success_reward, flag_invalid_goal,flag_invalid_joints, flag_path_tolerance_violation, flag_goal_tolerance_violation)
                state = state_
                ...
                if (flag_path_tolerance_violation == 1) or (flag_invalid_joints == 1):
                    state, object_position = env.reset()
                if (terminal==True) and (flag_goal_tolerance_violation == 0):
        ...
        with open(joint_angles_filename, "a") as file:
            file.write(f"joint_angles: {present_joint_angles} \n")

        with open(rpy_filename, "a") as file:
            file.write(f"rpy: {rpy} \n")
    ...
    for epoch in range(args.test_epoch):
        ...
        for step in range(args.test_step):
            ...
            if args.model_name == 'TD3':
                ...
                state_, reward, terminal, target_pos, end_eff_pos, present_joint_angles, rpy, flag_success_reward, flag_invalid_goal,flag_invalid_joints, flag_path_tolerance_violation, flag_goal_tolerance_violation = env.step(action, object_pos)
                state = state_
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_date', default='/28_08_2024')
    #Folder stored with trained model weights, which are used for transfer learning
    parser.add_argument('--model_date_', default='/28_08_2024')
