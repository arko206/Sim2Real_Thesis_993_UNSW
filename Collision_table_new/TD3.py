...
class TD3(object):
    ...
    def Learn(self):
        ...
        noise = torch.normal(mean=0, std=self.target_noise, size=a.size()).to(device)

        # Clip the noise to avoid large deviations
        noise_clamped = torch.clamp(noise, min=-self.noise_clip, max=self.noise_clip)

        ### chossing the action perfromed by the target actor network
        a_ = (self.actor_target(s_) + noise_clamped).detach()

        
        # a_ = torch.clamp(a_, min=-0.07,max=0.07).detach()
