...
class Actor(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.forward1 = nn.Linear(s_dim, 400)  # 将s_dim设为8
        self.Relu = nn.ReLU()
...
class DDPG(object):
    ...
    def save_model(self, model_dir, model_name):
        ...
        torch.save(self.optim_a.state_dict(),
                   model_dir+model_name+'optim_a.pth')
        torch.save(self.optim_c.state_dict(),
                   model_dir+model_name+'optim_c.pth')

    def load_model(self, model_dir, model_name):
        self.actor_target.load_state_dict(
            torch.load(model_dir+model_name+'actor.pth'))
        self.critic_target.load_state_dict(
            torch.load(model_dir+model_name+'critic.pth'))
        self.actor.load_state_dict(
            torch.load(model_dir+model_name+'actor.pth'))
        self.critic.load_state_dict(
            torch.load(model_dir+model_name+'critic.pth'))
        self.optim_a.load_state_dict(
            torch.load(model_dir+model_name+'optim_a.pth'))
        self.optim_c.load_state_dict(
            torch.load(model_dir+model_name+'optim_c.pth'))
