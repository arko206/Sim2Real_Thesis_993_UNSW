...
class Ur5():
    ...
    def step(self,action, object_position, timestep):
        ...
        try:
            ...
            ###angular velocities of all joint anghles
            # present_angular_velocities = joint_state_msg.velocity
        ...
        ### adding the present predicted velocity
        #### with earlier velocity
        # for i in range(len(action_sent)):
        #     action_sent[i] = action[i] + joint_current_velocities[i]
        
        ##joint angles are added
        for i in range(len(action)):
            action_sent[i] = action[i] + present_joint_angles[i]
        ...
        for i in range(6):
            if action_sent[i] > np.pi:
                action_sent[i] = action_sent[i] % -np.pi
    ...
    def get_state(self,angular_values, position, object_position):
        ...
        for i in range(len(angular_values)):
            angular_values[i] = angular_values[i]/(2 * 3.14)
        
        state = np.concatenate((angular_values, dx, dy, dz),axis=None)
