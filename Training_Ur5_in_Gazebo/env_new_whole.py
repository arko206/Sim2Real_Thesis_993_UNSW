...
class Ur5():
    ...
    def step(self,action, object_position):
        ...
        base_x, base_y, base_z = 0, 0, 0  # Base link position
        min_height = 0.80  # Minimum height constraint
        ...
        for i in range(6):
            ...
            # Ensure joint angles are greater than base link coordinates
            action_sent[i] = max(action_sent[i], base_x, base_y, base_z)
        
        # Apply specific joint constraints
        action_sent[0] = max(0, action_sent[0])  # shoulder_pan_joint: no negative x-axis movement
        action_sent[1] = np.clip(action_sent[1], -1, 1)  # shoulder_lift_joint: limit y-axis movement to [-1, 1]
        
        def forward_kinematics(joints):
            # This is a simplified FK. You may need to implement a more accurate version.
            shoulder_offset = 0.089159
            upper_arm_length = 0.425
            forearm_length = 0.39225
            wrist_1_length = 0.09465
            wrist_2_length = 0.09465
            
            x = (upper_arm_length * np.cos(joints[1]) + 
                forearm_length * np.cos(joints[1] + joints[2]) +
                wrist_1_length * np.cos(joints[1] + joints[2] + joints[3]) +
                wrist_2_length * np.cos(joints[1] + joints[2] + joints[3] + joints[4])) * np.cos(joints[0])
            
            y = (upper_arm_length * np.cos(joints[1]) + 
                forearm_length * np.cos(joints[1] + joints[2]) +
                wrist_1_length * np.cos(joints[1] + joints[2] + joints[3]) +
                wrist_2_length * np.cos(joints[1] + joints[2] + joints[3] + joints[4])) * np.sin(joints[0])
            
            z = (shoulder_offset + 
                upper_arm_length * np.sin(joints[1]) + 
                forearm_length * np.sin(joints[1] + joints[2]) +
                wrist_1_length * np.sin(joints[1] + joints[2] + joints[3]) +
                wrist_2_length * np.sin(joints[1] + joints[2] + joints[3] + joints[4]))
            
            return x, y, z
        
        # Check and adjust for constraints
        x, y, z = forward_kinematics(action_sent)
        
        # Ensure x is not negative
        if x < 0:
            action_sent[0] = 0  # Reset shoulder_pan_joint to prevent negative x
        
        # Ensure y is within [-1, 1]
        if abs(y) > 1:
            action_sent[1] = np.clip(action_sent[1], -1, 1)  # Adjust shoulder_lift_joint
        
        # Ensure z is above min_height
        if z < min_height:
            # Adjust shoulder_lift_joint and elbow_joint to increase height
            action_sent[1] += 0.1
            action_sent[2] -= 0.1
        ...
        state = self.get_state(action, position, object_position)
        reward, terminal, target_pos, end_eff_pos = self.get_reward(object_position, position)
    ...
    def reset(self):
        ...
        state_obtained = self.get_state([0,0,0,0,0],position, object_position)
    ...
    def get_state(self,action, position, object_position):
        ...
        state = np.concatenate((action, pose_6),axis=None)
        #state = np.concatenate((pose_6,goal_dis,action,in_point),axis=None)
        #state = state / np.linalg.norm(state)

        return state
    
    def get_reward(self,object_position,pos):
        t = False
    ...
    def target_vis(self,goal):
        ...
        #for row in [1]:
            #for col in range(1):
                ...
                #delete_model(reel_name)
                #pose = deepcopy(origin_pose)
                #pose.position.x = origin_pose.position.x + 0.28

                #if pose.position.x > 0.67:
                   #pose.position.x = 0.506

                #pose.position.y = origin_pose.position.y + 0.81

                #if pose.position.y > 0.65:
                    #pose.position.y = 0
